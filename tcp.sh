...
sh_ver="1.2.1"
github="raw.githubusercontent.com/bczol/Linux-NetSpeed/master"
...
startlotserver(){
	...
	wget --no-check-certificate -O appex.sh https://raw.githubusercontent.com/bczol/serverSpeeder_Install/master/appex.sh && chmod +x appex.sh && bash appex.sh install
	rm -f appex.sh
	start_menu
}
...
remove_all(){
	...
	if [[ -e /appex/bin/serverSpeeder.sh ]]; then
		wget --no-check-certificate -O appex.sh https://raw.githubusercontent.com/bczol/serverSpeeder_Install/master/appex.sh && chmod +x appex.sh && bash appex.sh uninstall
		rm -f appex.sh
	fi
	clear
	echo -e "${Info}:清除加速完成。"
